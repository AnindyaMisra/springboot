...
@SpringBootTest
//@WebAppConfiguration
public class Casestudy1ApplicationTests {
	...
	public void getCustomerTest() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.get("/customer").accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
	}
	...
	public void addCustomerTest() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.get("/customer/"+json.get("customerId")).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
		.andExpect(jsonPath("$").isNotEmpty())
...
//		.content(toJson(customer))
		...
		mvc.perform(MockMvcRequestBuilders.delete("/customer/100")
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());

		//confirm deletion
		mvc.perform(MockMvcRequestBuilders.get("/customer/"+customerId).accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
	}
	...
	public void updateCustomerTest() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.put("/customer/" + customerId)
		.content(toJson(customer))
		.contentType(MediaType.APPLICATION_JSON)
		.accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
		
		mvc.perform(MockMvcRequestBuilders.put("/customer/100")
		.content(toJson(customer))
		.contentType(MediaType.APPLICATION_JSON)
		.accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());

		//confirm Update
		mvc.perform(MockMvcRequestBuilders.get("/customer/"+customerId).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
		.andExpect(jsonPath("$").isNotEmpty())
		.andExpect(jsonPath("$.customerId").value(json.get("customerId")))
